...
async def meeting_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not text_args or "|" not in text_args:
        await update.message.reply_text(
            "⚠️ **ទម្រង់នៃការប្រើប្រាស់៖**\n\n"
            "សូមវាយបញ្ចូលតាមទម្រង់នេះ៖\n"
    ...
    try:
        ...
        if len(parts) < 3:
            await update.message.reply_text("⚠️ សូមបំពេញព័ត៌មានឱ្យបានครบ ៣ ចំណែក (ប្រធានបទ | ថ្ងៃម៉ោង | នាទីរំលឹក)")
            return
        ...
        await update.message.reply_text("❌ មានបញ្ហា៖ សូមពិនិត្យទម្រង់ថ្ងៃខែឆ្នាំ (DD-MM-YYYY HH:MM) និងតួលេខនាទីឡើងវិញ។")
